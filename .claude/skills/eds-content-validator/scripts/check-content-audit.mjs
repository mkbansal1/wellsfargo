#!/usr/bin/env node
/**
 * EDS Site Audit: Content completeness, media/assets, links, and quality checks.
...
async function fetchHtml(url, auth = AUTH, timeout = 20000) {
  const headers = {
...
async function headCheck(url, auth = null, timeout = 10000) {
...
for (let i = 0; i < urls.length; i += CONCURRENCY) {
  ...
  const batchResults = await Promise.all(batch.map(async (originalUrl) => {
    ...
    const { issues, assetUrls } = auditPage(html);
...
const csvHeader = [
  ...
  'issues_count', 'completeness_issues', 'image_issues', 'link_issues', 'quality_issues', 'video_issues',
  'issues',
].join(',');
