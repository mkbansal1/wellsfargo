...
if (!SITEMAP_JSON || !PROD_BASE || !EDS_BASE || !OUTPUT_DIR) {
  ...
  console.error('Options: --threshold=90 --concurrency=5 --max=N --offset=N --auth-prod=user:pass --auth-eds=user:pass');
  process.exit(1);
}
...
async function fetchHtml(url, auth) {
  const headers = {
...
async function processUrl(url, idx) {
  ...
  const [prodRes, edsRes] = await Promise.all([
    fetchHtml(prodUrl, AUTH_PROD),
    fetchHtml(edsUrl,  AUTH_EDS),
  ]);
