...
if (!SITEMAP_JSON || !PROD_BASE || !EDS_BASE || !OUTPUT_DIR) {
  ...
  console.error('Options: --threshold=90 --concurrency=2 --max=N --offset=N --auth-prod=user:pass --auth-eds=user:pass');
  process.exit(1);
}
...
async function processUrl(browser, url, idx) {
  ...
  if (AUTH_PROD) {
    const [user, pass] = AUTH_PROD.split(':');
  ...
  // Fetch EDS
  const edsCtxOpts = { viewport: { width: 1440, height: 900 } };
  if (AUTH_EDS) {
    const [user, pass] = AUTH_EDS.split(':');
    edsCtxOpts.httpCredentials = { username: user, password: pass };
  }
