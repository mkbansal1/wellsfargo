...
12. **Leaving-site interstitial is automatic** — Off-site links show a "You are leaving wellsfargo.com" confirmation popup before navigating, handled globally by `scripts/leaving-site.js` (loaded in the lazy phase). Authors do NOT mark links manually and there is no `data-href-id` indirection — just keep the real external URL on the link's `href` (absolute, per Rule 4). Any absolute `http(s)` link whose host is off-origin and NOT on the exclusion list triggers the popup. The exclusion list (hosts that pass through silently) lives in the `leaving-site-allow` key of `placeholders.json` and matches by **exact host**, so excluding `wellsfargo.com`/`www.wellsfargo.com` still lets first-party subdomains (`connect.secure.wellsfargo.com`, `stories.wf.com`, `creditcards.wellsfargo.com`, etc.) correctly show the interstitial (AGENTS.md Rule 7). Interstitial copy (`leaving-site-title`, `leaving-site-body` with a `{domain}` token, `leaving-site-continue`, `leaving-site-cancel`) is localized in `placeholders.json` (sheets `en`/`es`); the script ships built-in EN/ES defaults so it works before the sheet is published.
