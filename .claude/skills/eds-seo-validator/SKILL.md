...
- **401 returned** → site requires HTTP Basic Auth. Ask the user:
  > "The site returned 401 (HTTP Basic Auth / htaccess protected). Please provide credentials in `user:password` format."
  Then pass `--auth=user:password` to whichever script you run.
- **Any other status** → proceed without `--auth`.

**Note:** The CWV script (`check-cwv.mjs`) cannot use htaccess credentials — PageSpeed Insights crawls from Google's servers. Warn the user if they try to run CWV against a 401-protected URL.
...
## Mode 1: Fast SEO Check (`check-seo.js`)
