...
if (!sitemapJsonFile || !baseUrl || !outputCsvFile) {
  console.error('Usage: node check-seo-deep.js <sitemap-json> <base-url> <output-csv> [--auth=user:pass]');
  process.exit(1);
}
...
const httpCredentials = authFlag
...
async function checkPage(context, originalUrl, targetUrl, isHomepage) {
  ...
  if (!dom.schemas.some(s => s.type === 'WebSite'))  issues.push('MISSING: WebSite schema (§2.1)');
  if (!isHomepage && !dom.schemas.some(s => s.type === 'BreadcrumbList'))
  ...
  if (!isHomepage && !dom.hasBreadcrumb)
                               issues.push('MISSING: breadcrumb navigation (§7.6)');
  ...
  if (!dom.favicon)            issues.push('MISSING: favicon (§11.4)');
